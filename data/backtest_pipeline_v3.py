"""
Pipeline de backtesting v3 -- fuentes de datos mixtas + ACUMULACIÓN HISTÓRICA.

CAMBIO vs. la v2 (backtest_pipeline_v2.py): cada corrida ya NO sobreescribe
los resultados anteriores. Los casos detectados se guardan en un archivo
permanente (`backtest_history_store.json`), identificados por
ticker+estrategia+fecha (no por posición), así que correrlo de nuevo en
un mes se SUMA a lo que ya había en vez de reemplazarlo. Como el dato de
30 minutos solo cubre los últimos 60 días, cada corrida mensual capta una
ventana nueva que se solapa parcialmente con la anterior -- con el
tiempo, esto hace crecer la muestra real de VV11 / V10_roja / Falso_Gap
más allá del límite de una sola corrida.

Las estrategias que ya usan ventanas largas (SMA_hora a 730 días,
SMA_dia a 5 años) no necesitan esto para crecer -- cada corrida ya trae
casi todo el histórico disponible -- pero pasan por el mismo mecanismo
sin problema (simplemente no habrá casos nuevos que sumar de una corrida
a otra, más allá de los días que avanzó el calendario).

Los stats (`backtest_stats.json`) siempre se calculan sobre el
acumulado COMPLETO, no solo sobre la corrida más reciente.

CAMBIOS de la v2 vs. la v1 original (siguen aplicando en la v3):
...
HISTORY_STORE = "backtest_history_store.json"
MAX_CASOS_POR_ESTRATEGIA_UI = 60  # tope de casos que se exponen al modo de revisión


def cargar_historico(path=HISTORY_STORE):
    """Devuelve un dict {id: caso} con todo lo acumulado hasta ahora."""
...
def guardar_historico(historico_dict, path=HISTORY_STORE):
    with open(path, "w", encoding="utf-8") as f:
        json.dump(list(historico_dict.values()), f, ensure_ascii=False, indent=2)
...
def main():
    ...
    for ticker in TICKERS:
        ...
        for caso in crudos:
            ...
            ts_iso = pd.Timestamp(caso["ts"]).isoformat()
            todos.append({
                "id": f"{ticker}-{caso['estrategia']}-{ts_iso}",
        ...
        for c in detectar_sma_hora_serie(df1h):
            ts_iso = pd.Timestamp(c["ts"]).isoformat()
            todos.append({"id": f"{ticker}-SMA_hora-{ts_iso}", "ticker": ticker,
        ...
        for c in detectar_sma_dia_serie(df_diario, df1h):
            ts_iso = pd.Timestamp(c["ts"]).isoformat()
            todos.append({"id": f"{ticker}-SMA_dia-{ts_iso}", "ticker": ticker,
                           "fecha": str(c["ts"]), **{k: v for k, v in c.items() if k != "ts"}})

    print(f"\nCasos detectados en ESTA corrida: {len(todos)}")

    # --- acumular contra el histórico permanente (merge por id, no se pierde nada) ---
    historico = cargar_historico()
    casos_nuevos_reales = sum(1 for c in todos if c["id"] not in historico)
    for c in todos:
        historico[c["id"]] = c  # si ya existía, se refresca con el recálculo más reciente
    guardar_historico(historico)
    print(f"Casos realmente nuevos (no vistos en corridas anteriores): {casos_nuevos_reales}")
    print(f"Total acumulado en {HISTORY_STORE}: {len(historico)}")

    todos_acumulados = list(historico.values())
    ...
    for nombre, direccion in ESTRATEGIAS_DIR.items():
        casos = [c for c in todos_acumulados if c["estrategia"] == nombre]
        n_casos = len(casos)
        if n_casos == 0:
    ...
    with open("backtest_stats.json", "w", encoding="utf-8") as f:
        json.dump({"generado": pd.Timestamp.now(tz="America/New_York").isoformat(),
                    "fuentes": {"VV11": "30min (60d, acumulado entre corridas)",
                                 "V10_roja": "30min (60d, acumulado entre corridas)",
                                 "Falso_Gap": "30min (60d, acumulado entre corridas)",
                                 "SMA_hora": "1h (730d)",
                                 "SMA_dia": "diario (5y) + 1h para confirmación"},
                    "tickers": TICKERS, "estrategias": stats}, f, ensure_ascii=False, indent=2)

    # muestra para el modo de revisión: si hay más de las que caben, tomar
    # una muestra aleatoria (semilla fija) para variedad en vez de solo las
    # más recientes -- así no se repite siempre el mismo subconjunto
    rng = random.Random(42)
    casos_finales = []
    for nombre in ESTRATEGIAS_DIR:
        casos_estrategia = [c for c in todos_acumulados if c["estrategia"] == nombre]
        if len(casos_estrategia) > MAX_CASOS_POR_ESTRATEGIA_UI:
            casos_estrategia = rng.sample(casos_estrategia, MAX_CASOS_POR_ESTRATEGIA_UI)
        casos_finales.extend(casos_estrategia)
    ...
    print(f"\nGuardado: backtest_stats.json, backtest_cases.json ({len(casos_finales)} casos "
          f"muestreados del acumulado), {HISTORY_STORE} ({len(historico)} casos totales)")
